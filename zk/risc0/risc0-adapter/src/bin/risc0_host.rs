//! risc0-host: the same CLI protocol as sp1-host, second implementer.
//!
...
fn main() {
    ...
    if let Err(error) = run(&args) {
        eprintln!("risc0-host error: {error}");
        std::process::exit(2);
    }
}

fn setup(elf_path: &str, descriptor_path: &str) -> Result<Risc0KernelBackend, String> {
    ...
    Risc0KernelBackend::setup(elf, binding).map_err(|e| format!("setup: {e:?}"))
}
...
fn prove(args: &[String]) -> Result<(), String> {
    ...
    out.push_str(&format!(
        "native_commitment image_id {}\n",
        backend.image_id_hex()
    ));
    out.push_str(&format!(
        "input_commitment {}\n",
